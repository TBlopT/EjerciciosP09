...
Cuando se declara una constante usando la etiqueta `const`, el compilador comprobará implícitamente si se trata de una constante *en tiempo de compilación* o 
*en tiempo de ejecución*. 
En la mayoría de los casos, esto sólo es relevante para la optimización del código, pero hay casos en los que C++ requiere una expresión constante que sólo 
...
const int kMyConst1{x};        // constante en ejecución, su inicializador no es constante
const int kMyConst2{5};        // constante en compilación, su inicializador es una expresión constante
const int kResult{getValue()}; // no está claro. Depende de cómo se haya definido getValue()
```

Afortunadamente, se puede indicar al compilador cuando se necesita que una constante sea evaluable en tiempo de compilación. 
Para ello, se usa la etiqueta `constexpr` en lugar de `const`. 
Una `constexpr` ("expresión constante") sólo puede ser evaluada en tiempo de compilación. 
Si su inicializador no es una expresión constante, el compilador generará un error.
...
Téngase en cuenta que algunos tipos de datos que reservan memoria dinámicamente, tales como `std::string` y `std::vector`, no son compatibles con `constexpr`. 
Para constantes de estos tipos, se debe usar `const`. 
Asimismo, los parámetros de las funciones siempre se evalúan en tiempo de ejecución y, por tanto, no pueden ser declaradas como `constexpr`.

En algunos contextos, el compilador puede decidir que no es necesario evaluar una expresión en tiempo de compilación.

```
constexpr int kIntensidad{3 + 4};  // 3 + 4 siempre se evaluará en tiempo de compilación
...
int main() {
  constexpr int kValor{3 + 4};    // 3 + 4 es una expresión constante
  std::cout << kValor;            // Se evalúa en tiempo de ejecución
  return 0;
}
```

`3 + 4` es una expresión constante, así que el compilador la evaluará en tiempo de compilación y la sustituirá por el valor `7`. 
Pero como `kValor` es también una constante en tiempo de compilación, el compilador probablemente optimizará el programa sustituyendo 
`std::cout << kValor` por `std::cout << 7`, que se ejecutará en tiempo de ejecución. 
Sin embargo, como `kValor` sólo se usa una vez, es más probable que el compilador optimice el programa eliminando `kValor` antes de evaluar `3 + 4`:
...
La expresión `std::cout << 3 + 4` ya no es constante, así que surge la duda de si la subexpresión `3 + 4` se evaluaría en tiempo de compilación o no. 
En general sí se optimizará, ya que los compiladores son capaces de detectar subexpresiones constantes incluso si son parte de expresiones en tiempo de ejecución. 
A este proceso de optimización se le conoce como *"constant folding"* ("plegado de constantes"). 
Etiquetar las expresiones constantes como `constexpr` asegura que dichos valores sean reconocidos por el plegado de constantes siempre que estas se usen 
en una expresión en tiempo de ejecución.
...
* Automatice la compilación del programa correspondiente a cada ejercicio utilizando la herramienta cmake.
...
espacios y la modifique de forma que queden las consonantes en minúscula y las vocales en mayúscula.
...
5. Desarrolle un programa `hypotenuse.cc` que incluya una función que tome como parámetros los catetos de un
